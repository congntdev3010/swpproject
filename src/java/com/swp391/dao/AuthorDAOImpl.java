...
public class AuthorDAOImpl implements AuthorDAO {
    ...
    public Author create(Author author) throws Exception {
        String sql = "INSERT INTO authors (name, nationality, birth_date, bio, avatar_url, created_at) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public Author findById(int id) throws Exception {
        String sql = "SELECT id, name, nationality, birth_date, bio, avatar_url, created_at FROM authors WHERE id = ?";
        try (Connection con = DBContext.getInstance().getConnection(); 
    ...
    public List<Author> findAll() throws Exception {
        String sql = "SELECT id, name, nationality, birth_date, bio, avatar_url, created_at FROM authors ORDER BY name";
    ...
    public Author findByName(String name) throws Exception {
        String sql = "SELECT id, name, nationality, birth_date, bio, avatar_url, created_at FROM authors WHERE name = ?";
        try (Connection con = DBContext.getInstance().getConnection(); 
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, name);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public boolean update(Author author) throws Exception {
        String sql = "UPDATE authors SET name = ?, nationality = ?, birth_date = ?, bio = ?, avatar_url = ? WHERE id = ?";
        try (Connection con = DBContext.getInstance().getConnection(); 
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, author.getName());
            ps.setString(2, author.getNationality());
