...
<div class="container" style="padding-top:28px;">
    ...
    <div class="book-detail-layout">
        ...
        <div class="book-detail-cover-section">
            ...
            <% } %>
            <%-- Nút mượn sách cho READER --%>
            <% if (loggedUser != null && loggedUser.isReader() && book != null && book.getAvailable() > 0) { %>
            <form method="post" action="<%= ctx %>/borrow" style="margin-top:10px;">
                <input type="hidden" name="action" value="add">
                <input type="hidden" name="bookId" value="<%= book.getId() %>">
                <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center; border-radius:var(--radius-md); padding:12px;">
                    <i class="fa-solid fa-cart-shopping"></i> Thêm vào giỏ mượn
                </button>
            </form>
            <% } else if (loggedUser != null && loggedUser.isReader() && book != null && book.getAvailable() <= 0) { %>
            <div style="margin-top:10px; padding:10px 14px; background:rgba(255,71,87,0.08); border:1px solid rgba(255,71,87,0.2); border-radius:var(--radius-sm); text-align:center; font-size:0.85rem; color:var(--danger); font-weight:500;">
                <i class="fa-solid fa-ban"></i> Hết sách để mượn
            </div>
            <% } %>
        </div>
