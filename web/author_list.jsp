                ...
                <nav aria-label="Phân trang">
                    <ul class="pagination" style="margin-top: 0;">
                        ...
                        <li class="page-item <%= currentPageNum <= 1 ? "disabled" : "" %>">
                            <a class="page-link"
                               href="<%= ctx %>/authors?keyword=<%= java.net.URLEncoder.encode(keyword,"UTF-8") %>&sort=<%= sortField %>&order=<%= sortOrder %>&page=<%= currentPageNum - 1 %>">
                           ...
                           if (totalPages <= 7) {
                               for (int pg = 1; pg <= totalPages; pg++) { %>
                                   <li class="page-item <%= pg == currentPageNum ? "active" : "" %>">
                                       <a class="page-link" href="<%= ctx %>/authors?keyword=<%= java.net.URLEncoder.encode(keyword,"UTF-8") %>&sort=<%= sortField %>&order=<%= sortOrder %>&page=<%= pg %>"><%= pg %></a>
                                   </li>
                               <% }
                           } else {
                               // Show first 2 pages
